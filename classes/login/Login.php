...
class Login
{
        ...
        // if user has an active session on the server
            ...
            // user try to change his email
                ...
                $this->editUserEmail($_POST['user_email']);
						// user changes name
            } elseif (isset($_POST["user_edit_submit_firstlast_name"])) {
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT user_id, user_name, user_email, user_first_name, user_last_name, role, location_id FROM users WHERE user_id = :user_id
                    ...
                    if (isset($result_row->user_id)) {
                        ...
                        $_SESSION['user_location'] = $result_row->location_id;
                        ...
                        $this->user_first_name = $result_row->user_first_name;
                        $this->user_last_name = $result_row->user_last_name;
                        $this->user_is_logged_in = true;
    ...
    {
        // if database connection opened
				// Edit by CH @ Vertical Motion due to errors from expired sessions
        // if ($this->databaseConnection()) {
    ...
    {
        // prevent database flooding
        $user_first_name = substr(trim($user_first_name), 0, 32);
        ...
        if (empty($user_first_name)) {
            $this->errors[] = MESSAGE_NAME_INVALID;
						...
						$query_edit_user_first_name = $this->db_connection->prepare('UPDATE users SET user_first_name = :user_first_name WHERE user_id = :user_id');
						$query_edit_user_first_name->bindValue(':user_first_name', $user_first_name, PDO::PARAM_STR);
						$query_edit_user_first_name->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
						$query_edit_user_first_name->execute();

						if ($query_edit_user_first_name->rowCount()) {
								$_SESSION['user_first_name'] = $user_first_name;
								$this->messages[] = MESSAGE_NAME_CHANGED_SUCCESSFULLY;
						} else {
								$this->errors[] = MESSAGE_NAME_CHANGE_FAILED;
						}
        }
    }
    ...
    {
        // prevent database flooding
        $user_last_name = substr(trim($user_last_name), 0, 32);
        ...
        if (empty($user_last_name)) {
            $this->errors[] = MESSAGE_NAME_INVALID;
						...
						// write users new data into database
						$query_edit_user_last_name = $this->db_connection->prepare('UPDATE users SET user_last_name = :user_last_name WHERE user_id = :user_id');
						$query_edit_user_last_name->bindValue(':user_last_name', $user_last_name, PDO::PARAM_STR);
						$query_edit_user_last_name->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
						$query_edit_user_last_name->execute();

						if ($query_edit_user_last_name->rowCount()) {
